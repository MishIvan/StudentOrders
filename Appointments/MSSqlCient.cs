...
namespace Appointments
{
    public class MSSqlCient : IDisposable
    {
        ...
        /// </summary>
        /// <param name="uList">списос пользователей</param>
        /// <returns></returns>
        public bool fillUserList(IList<String> uList)
        {
            if(!isOpened) return false;

            uList.Clear();
            SqlCommand cmd = new SqlCommand("select Name from dbo.Users", conn);
            SqlDataReader rd = cmd.ExecuteReader();
            while (rd.Read())
            {
                uList.Add(rd.GetString(0));
            }
            rd.Close();
            return true;
        }
